-- Aggregate commerce panel statistics inside Postgres.
-- Used by server-side panel APIs through the service role client only.
...
as $$
  ...
  summary_metrics as (
    select
      ...
      count(*) filter (where coalesce(status, '') not in ('completed', 'cancelled'))::bigint as in_progress_orders
    from scoped_orders
  ),
  billable_metrics as (
    select
  ...
  sales_by_day_rows as (
    select
      to_char(created_at_ve, 'YYYY-MM-DD') as label,
      coalesce(sum(total_usd), 0)::numeric as value
    from billable_orders
    group by 1
  ),
  ...
  select
    ...
    coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'revenue', revenue,
        'orders', orders,
        'share', case when coalesce(billable_metrics.total_revenue_usd, 0) > 0 then (revenue / billable_metrics.total_revenue_usd) * 100 else 0 end
