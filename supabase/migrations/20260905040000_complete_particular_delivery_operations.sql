...
create unique index if not exists transport_particular_requests_agency_idempotency_idx
  on public.transport_particular_requests(agency_id, idempotency_key)
...
begin
  ...
  if v_order_id is not null and p_order_delivery_status is not null then
  ...
  if v_order_id is not null and p_integration_status is not null then
...
comment on column public.transport_particular_requests.idempotency_key is
  'Identificador de reintento del navegador para impedir solicitudes duplicadas.';
