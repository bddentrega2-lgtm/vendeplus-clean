-- Exclude cancelled orders from operational statistics.
-- Cancelled orders remain visible as their own counter/status, but they no longer
-- contribute to total order volume, revenue, averages, charts or admin metrics.
...
as $$
  ...
  select
    (select count(*) from billable_orders)::bigint as total_orders,
    (select count(*) from billable_orders, bounds where billable_orders.created_at >= bounds.today_start)::bigint as orders_today,
    (select count(*) from billable_orders, bounds where billable_orders.created_at >= bounds.seven_days_ago)::bigint as orders_last_7_days,
    coalesce((select sum(total_usd) from billable_orders), 0)::numeric as revenue_usd,
...
as $$
  ...
  select
    ...
    (select count(*) from billable_orders)::bigint as total_orders,
    (select count(*) from billable_orders where billable_orders.created_at >= now() - interval '7 days')::bigint as orders_last_7_days,
    (select count(*) from billable_orders where billable_orders.created_at >= now() - interval '30 days')::bigint as orders_last_30_days,
    coalesce((select sum(total_usd) from billable_orders), 0)::numeric as total_revenue_usd,
...
as $$
  ...
  billable_metrics as (
    select
      ...
      coalesce(sum(total_usd), 0)::numeric as total_revenue_usd,
      ...
      coalesce(sum(total_usd) filter (where delivery_type = 'delivery'), 0)::numeric as delivery_revenue_usd,
      coalesce(sum(total_usd) filter (where delivery_type = 'pickup'), 0)::numeric as pickup_revenue_usd,
      ...
      coalesce(sum(total_usd) filter (where safe_payment_status in ('pending', 'review', 'incomplete')), 0)::numeric as pending_payment_usd,
      count(*) filter (
  ...
  top_customer_rows as (
    select
      coalesce(nullif(customer_phone, ''), 'Sin teléfono') as phone,
      coalesce(max(customer_name), 'Cliente') as customer,
      count(*)::bigint as orders,
      coalesce(sum(total_usd), 0)::numeric as revenue,
      max(created_at)::text as last_order_at
    from billable_orders
    group by 1
  ),
  sales_by_day_rows as (
    select to_char(created_at_ve, 'YYYY-MM-DD') as label, coalesce(sum(total_usd), 0)::numeric as value
    from billable_orders
    group by 1
  ),
  ...
  sales_by_week_rows as (
    select to_char(created_at_ve, 'IYYY-"W"IW') as label, coalesce(sum(total_usd), 0)::numeric as value
    from billable_orders
    group by 1
  ),
  sales_by_month_rows as (
    select to_char(created_at_ve, 'YYYY-MM') as label, coalesce(sum(total_usd), 0)::numeric as value
    from billable_orders
    group by 1
  ),
  ...
  revenue_by_store_rows as (
    select store_name as label, coalesce(sum(total_usd), 0)::numeric as value
    from billable_orders
    group by 1
  ),
