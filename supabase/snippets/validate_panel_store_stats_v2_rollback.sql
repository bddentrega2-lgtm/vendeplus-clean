...
begin
  ...
  if v_store_id is null then
    raise exception 'No hay comercio con pedidos para validar estadisticas.';
  end if;

  select *
    into v_stats
  from public.panel_store_stats(array[v_store_id], v_store_id, v_start, v_end, 8);

  select
    count(*)::bigint,
    coalesce(sum(coalesce(orders.subtotal_usd, greatest(coalesce(orders.total_usd, 0) - coalesce(orders.delivery_usd, 0), 0))), 0)::numeric
    into v_expected_orders, v_expected_revenue
  from public.orders
  where orders.store_id = v_store_id
    and orders.created_at >= v_start
    and orders.created_at <= v_end
    and lower(coalesce(orders.status, '')) not in ('cancelled', 'canceled', 'cancelado');

  if coalesce((v_stats.summary->>'aggregationVersion')::integer, 0) <> 2 then
    raise exception 'Version de agregacion incorrecta.';
  end if;

  if coalesce((v_stats.summary->>'totalOrders')::bigint, -1) <> v_expected_orders then
    raise exception 'Conteo agregado no coincide.';
  end if;

  if abs(coalesce((v_stats.summary->>'totalRevenueUsd')::numeric, -1) - v_expected_revenue) > 0.0001 then
    raise exception 'Ingreso agregado no coincide.';
  end if;

  if jsonb_array_length(v_stats.recent_orders) > 8 then
    raise exception 'La lista reciente excede el limite.';
  end if;

  if has_function_privilege('anon', 'public.panel_store_stats(uuid[],uuid,timestamptz,timestamptz,integer)', 'EXECUTE')
     or has_function_privilege('authenticated', 'public.panel_store_stats(uuid[],uuid,timestamptz,timestamptz,integer)', 'EXECUTE')
     or not has_function_privilege('service_role', 'public.panel_store_stats(uuid[],uuid,timestamptz,timestamptz,integer)', 'EXECUTE') then
    raise exception 'Permisos de la RPC incorrectos.';
  end if;
end;
$$;
