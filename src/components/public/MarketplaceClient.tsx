...
function ProductRail({ title, eyebrow, products, badge }: { title: string; eyebrow?: string; products: MarketplaceProduct[]; badge: string }) {
  ...
  return <section className="vp-container py-2 sm:py-3"><div className={`rounded-[26px] p-4 shadow-sm ring-1 sm:p-5 ${tone}`}><div className="flex items-end justify-between gap-4"><div>{eyebrow ? <p className={`text-[10px] font-black uppercase tracking-[0.15em] ${eyebrowTone}`}>{eyebrow}</p> : null}<h2 className={eyebrow ? "mt-1 text-xl font-black leading-tight text-[#143D42] sm:text-2xl" : "text-xl font-black leading-tight text-[#143D42] sm:text-2xl"}>{title}</h2></div>{products.length > INITIAL_SECTION_ITEMS ? <button type="button" onClick={() => setExpanded((value) => !value)} className="shrink-0 rounded-full bg-white/80 px-4 py-2 text-xs font-black text-[#0F6B63] shadow-sm">{expanded ? "Ver menos" : "Ver todos"}</button> : null}</div><div className="vp-scrollbar-none mt-3 flex snap-x snap-mandatory gap-2.5 overflow-x-auto pb-1">{visible.map((product) => <ProductCard key={`${badge}-${product.productId}`} product={product} badge={badge} />)}</div></div></section>;
}
...
export function MarketplaceClient({ stores, featuredProducts = [], discovery = { offers: [], bestSellers: [], newProducts: [] }, eyebrow = "Marketplace", title = "Descubre que pedir hoy", description = "Las mejores opciones en un solo lugar.", storesEyebrow = "Todos los comercios", storesTitle = "Explora y elige", emptyTitle = "No encontramos comercios", emptyText = "Prueba con otra busqueda, rubro o zona.", footerText = "Marketplace de comercios afiliados.", partnerName, partnerLogoUrl, partnerBannerImageUrl, partnerLocation, partnerPrimaryColor, partnerAccentColor }: { stores: Store[]; featuredProducts?: MarketplaceFeaturedProduct[]; discovery?: MarketplaceDiscovery; eyebrow?: string; title?: string; description?: string; storesEyebrow?: string; storesTitle?: string; emptyTitle?: string; emptyText?: string; footerText?: string; partnerName?: string; partnerLogoUrl?: string | null; partnerBannerImageUrl?: string | null; partnerLocation?: string; partnerPrimaryColor?: string; partnerAccentColor?: string; }) {
  ...
  const filteredStores = useMemo(() => { const needle = normalizeSearch(query); const categoryFilter = !["Todos", "Abiertos", "Delivery", "Retiro", "Ofertas"].includes(activeFilter) ? normalizeSearch(activeFilter) : ""; return storesWithDistance.filter(({ store }) => { const text = storeSearchText(store); const productMatch = (productSearchByStore.get(store.id) || []).some((value) => value.includes(needle)); const specialMatch = activeFilter === "Abiertos" ? store.openState?.isOpen !== false : activeFilter === "Delivery" ? store.deliverySettings?.deliveryEnabled !== false : activeFilter === "Retiro" ? store.deliverySettings?.pickupEnabled !== false : activeFilter === "Ofertas" ? offerStoreIds.has(store.id) : true; const cityMatch = activeCity === "Todas" || store.citySlug === activeCity; return cityMatch && (!needle || text.includes(needle) || productMatch) && (!categoryFilter || text.includes(categoryFilter)) && specialMatch; }).sort((a, b) => coordinates ? Number(a.distance ?? Number.MAX_VALUE) - Number(b.distance ?? Number.MAX_VALUE) : a.store.name.localeCompare(b.store.name)); }, [activeCity, activeFilter, coordinates, offerStoreIds, productSearchByStore, query, storesWithDistance]);
  ...
  return <main id="inicio" style={marketplaceTheme} className="min-h-screen scroll-smooth bg-[#F3F1EC] pb-20 text-[#143D42] sm:pb-0">
    {cities.length ? <section className="vp-container pt-3"><label className="flex items-center gap-2 rounded-2xl bg-white p-3 shadow-sm ring-1 ring-[#143D42]/[0.06]"><MapPin size={17} className="shrink-0 text-[#FF7133]" /><span className="sr-only">Ciudad</span><select aria-label="Filtrar por ciudad" value={activeCity} onChange={(event) => setActiveCity(event.target.value)} className="h-10 min-w-0 flex-1 rounded-xl bg-[#E8F6F1] px-3 text-sm font-black text-[#0F6B63] outline-none"><option value="Todas">Todas las ciudades</option>{cities.map(([slug, name]) => <option key={slug} value={slug}>{name}</option>)}</select></label></section> : null}
    ...
    <section className="vp-container pt-3 sm:pt-6"><div className="relative overflow-hidden rounded-[28px] bg-[var(--marketplace-primary)] p-5 text-white shadow-lg shadow-[#143D42]/10 sm:p-8"><div className="absolute -right-20 -top-24 h-56 w-56 rounded-full bg-[var(--marketplace-accent)] opacity-35 blur-3xl" /><div className="absolute -bottom-24 left-1/3 h-44 w-44 rounded-full bg-[#FFD45C]/20 blur-3xl" />{partnerName && partnerBannerImageUrl ? <div className="relative mb-5 h-36 overflow-hidden rounded-[22px] sm:h-52"><OptimizedImage src={partnerBannerImageUrl} alt={`Banner de ${partnerName}`} fill sizes="1080px" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[var(--marketplace-primary)]/80 to-transparent" /></div> : null}<div className="relative max-w-3xl">{eyebrow ? <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#FFD45C]">{eyebrow}</p> : null}<h1 className={eyebrow ? "mt-1.5 text-[28px] font-black leading-[1.02] sm:text-5xl" : "text-[28px] font-black leading-[1.02] sm:text-5xl"}>{title}</h1>{description ? <p className="mt-2 max-w-xl text-sm font-semibold leading-relaxed text-white/70 sm:text-base">{description}</p> : null}</div>{partnerName ? <div className="relative mt-4 flex max-w-md items-center gap-3 border-l-2 border-[var(--marketplace-accent)] pl-3"><OptimizedImage src={partnerLogoUrl || ""} alt={partnerName} width={44} height={44} className="h-11 w-11 rounded-xl bg-white object-cover" /><div><p className="font-black">{partnerName}</p>{partnerLocation ? <p className="text-xs font-bold text-white/65">{partnerLocation}</p> : null}</div></div> : null}</div></section>
    ...
    {filteredFeaturedProducts.length ? <section className="vp-container py-2"><div className="rounded-[26px] bg-[#FFF0E8] p-4 ring-1 ring-[#FF7133]/10 sm:p-5"><div className="flex items-center gap-3"><Sparkles className="text-[#FF7133]" /><h2 className="text-xl font-black sm:text-2xl">Destacados Somos</h2></div><div className="vp-scrollbar-none mt-4 flex snap-x gap-3 overflow-x-auto pb-1">{filteredFeaturedProducts.map((product) => <ProductCard key={product.rewardId} badge="Destacado" product={{ productId: product.productId, storeId: product.storeId, storeName: product.storeName, storeSlug: product.storeSlug, productName: product.productName, description: product.description, imageUrl: product.imageUrl, priceUsd: product.priceUsd, discountPercent: product.discountPercent, createdAt: "" }} />)}</div></div></section> : null}
