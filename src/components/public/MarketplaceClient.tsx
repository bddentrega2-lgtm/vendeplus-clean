      ...
      {featuredProducts.length ? (
        <section className="border-y border-[#FFB547]/30 bg-gradient-to-br from-[#2E3A79] to-[#4656A4] text-white">
          <div className="vp-container py-5 sm:py-6">
            ...
            <div className="vp-scrollbar-none mt-4 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 md:grid md:grid-cols-2 md:overflow-visible md:pb-0">
              {featuredProducts.map((product) => {
                const finalPrice = product.discountPercent > 0 ? product.priceUsd * (1 - product.discountPercent / 100) : product.priceUsd;
                return <Link key={product.rewardId} href={`/${product.storeSlug}`} className="group grid min-h-32 w-[88%] shrink-0 snap-start grid-cols-[108px_1fr] overflow-hidden rounded-[20px] bg-white text-[#25262B] shadow-xl shadow-black/10 transition hover:-translate-y-0.5 sm:grid-cols-[126px_1fr] md:w-auto">
