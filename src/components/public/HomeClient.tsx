...
export function HomeClient({ stores = [], transportAgencies = [] }: { stores?: Store[]; transportAgencies?: PublicTransportAgencyLogo[] }) {
  ...
  return <main className="somos-page">
    ...
    <section className="bg-[var(--somos-navy)] py-14 text-white sm:py-20"><div className="vp-container grid gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
      ...
      <div className="rounded-3xl bg-white/8 p-6 ring-1 ring-white/10"><p className="text-sm font-semibold text-[var(--somos-amber)]">Pagas por resultado</p><div className="mt-2 flex items-end gap-2"><span className="text-5xl font-bold">$0.10</span><span className="pb-1 text-sm font-medium text-white/60">por pedido recibido</span></div><p className="mt-3 text-sm font-medium text-white/70">Sin mensualidades, contratos ni comisiones porcentuales.</p><p className="mt-2 text-xs font-semibold text-white/60">Envía tu solicitud. Somos revisará los datos antes de habilitar el acceso al panel.</p><ButtonLink href="/registro" variant="light" className="mt-6 w-full sm:w-fit">Solicitar acceso <ArrowRight size={17} /></ButtonLink></div>
